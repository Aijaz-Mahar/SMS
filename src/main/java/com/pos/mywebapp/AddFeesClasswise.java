 ...
 * @author Admin
 */
@WebServlet(name = "AddFeesClasswise", urlPatterns = {"/AddFeesClasswise"})
public class AddFeesClasswise extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
      ...
      PrintWriter out = response.getWriter();
            ...
            int result = 0;
             ...
             categoryId=request.getParameter("categories");
             }
                         
             if(request.getParameter("classes")!=null){
             classId=request.getParameter("classes");
             }
             if(request.getParameter("feeses")!=null){
             feesId=request.getParameter("feeses");
             }    
               if(request.getParameter("feesamount")!=null){
             feesAmount=request.getParameter("feesamount");
             }      
...
try{
    ...
    PreparedStatement ps = con.prepareStatement("insert into FeesStructure(categoryid,classid,feesid,amount) values(?,?,?,?)");
    ps.setString(1, categoryId);
    ps.setString(2, classId);
    ps.setString(3, feesId);
    ps.setDouble(4, famount);
